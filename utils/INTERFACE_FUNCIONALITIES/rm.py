...
class RMO_FUNCTIONS():
    ...
    def rmo_add(self, self2, w, tree_products, total_value_lb, operation = None):
            ...
            self2.prod_qt[self2.curr_prod_qt] = int(w.rmor_slc_p.rmor_slct_p_f4_lb.text())
            ...
            if (get_error == 0):
                ...
                prod_temp = [item.text(0),item.text(1),self2.prod_qt[self2.curr_prod_qt]]
                self.data_p = p.view()
                prod = self.data_p.loc[self.data_p['ID'] == int(prod_temp[0])]
                prod = prod.loc[prod['PRODUCT'] == prod_temp[1]]
                prod1 = prod.values.tolist()[0][5]
                ...
                if (operation == "Update"):
                        if (prod1 < prod_temp[2]):
                                ...
                                w.hide()
                                return 0
                prod1 = prod1 - prod_temp[2]
                p.update(self.data_p, "STOCK", prod, prod1)
                temp.append(item.text(1)) 
                temp.append(str(w.rmor_slc_p.rmor_slct_p_f4_lb.text()))
...
class Rmo_Update(RMO_FUNCTIONS):
    ...
    def rmou_btn(self, w):
        ...
        w.rmor_slc_p.rmor_slct_p_f4_btn_add.clicked.connect(lambda: self.rmo_add(self, w, w.rmou_p_slct_tree, w.rmou_slct_p_f1_tv, "Update"))
        w.rmor_slc_p.rmor_slct_p_f4_btn_l.clicked.connect(lambda: self.rmo_remove_qt(self, w))
        w.rmor_slc_p.rmor_slct_p_f4_btn_m.clicked.connect(lambda: self.rmo_add_qt(self, w))
