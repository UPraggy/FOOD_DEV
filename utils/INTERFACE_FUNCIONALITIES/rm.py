...
class Rmc_View(RMC_FUNCTIONS):
    ...
    def rmcv_btn(self, w):
        # SEARCH
        w.rmcv_slct_btn.clicked.connect(lambda: self.search_data(w.rmcv_slct_src.text(), w.rmcv_f_tree, self.data))
...
class Rmc_Update(RMC_FUNCTIONS):
    ...
    def rmcu_btn(self, w):
            ...
            # SEARCH
            w.rmcu_slct_btn.clicked.connect(lambda: self.search_data(w.rmcu_slct_src.text(), w.rmcu_f_tree, self.data))
...
class Rmc_Delete(RMC_FUNCTIONS):
    ...
    def rmcd_btn(self, w):
        ...
        # SEARCH
        w.rmcd_slct_btn.clicked.connect(lambda: self.search_data(w.rmcd_slct_src.text(), w.rmcd_f_tree, self.data))
...
class RMO_FUNCTIONS():
    def search_data(self, src_field_data, tree, data, field_src, type_data = None, more_field = None):
                ...
                for x in range(0, 2):
                        try:
                                if (str(filtered_data)[0:15] != 'Empty DataFrame'):
                                        pass
                                else:
                                    filtered_data = data[more_field[x]]
                                    filtered_data = data.loc[filtered_data.astype(str).str.contains(f'^{src_field_data}', case = False)]
                        except:
                            pass
    ...
    def switch_data(self, self2, w, date, operation = None, tree = None):
                if (self.data == "ERROR: Invalid Input"):
                    ...
                    for dt in rrule.rrule(rrule.MONTHLY, dtstart=start_date, until=end_date):
                        ...
                        self2.data.append(o.view(False, month, int(dt.year)))
                    if (operation == 'GRAPH'):
...
class Rmo_Register(RMO_FUNCTIONS):
    ...
    def finish_reg(self):
        ...
        month = f'{dtime.datetime.strftime(date, "%B")}'
...
class Rmo_Delete(RMO_FUNCTIONS):
    ...
    def init_Tree_widget(self, w):
            ...
            self.insert_tree_data(self.data, w.rmod_reg_f2_tree)
    ...
    def rmod_next_step(self, w):
            if (w.rmod_reg_f2_tree.currentItem() != None):
                    ...
                    self.slct_data = self.data.loc[self.data['number_order'] == int(self.slct_data)]
                    self.slct_data1 = w.rmod_reg_f2_tree.currentItem().text(1)
                    self.slct_data = self.slct_data.loc[self.slct_data['client'] == self.slct_data1]
